...
function initSchema(database: Database.Database): void {
  ...
  // Backfill the link where it is unambiguous. Multi-entry sessions need the
  // filename patterns to attribute, so an object file sync links those instead.
  try {
    database.exec(`
  ...
  // Give existing rows unique priorities when they all still share the default 0.
  ...
  // Seed a default rig, then tag untagged sessions and plans with it.
...
export const refreshDatabaseFromBlob = async (force: boolean): Promise<void> => {
  ...
  refreshing = (async () => {
    // Our own unsent changes go up first, or replacing the file discards them.
    await flushDatabaseToBlob()
